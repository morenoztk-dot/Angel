...
BOT_OWNER_ID = 1080030525542125579  # dono do bot
...
async def _remove_castigo_after(guild, member, castigo_role, delay=300):
    """Remove o cargo de castigo apos o tempo especificado (anti-spam)."""
    await asyncio.sleep(delay)
    try:
        member_fresh = guild.get_member(member.id)
        if member_fresh and castigo_role in member_fresh.roles:
            _pending_bot_actions.add((guild.id, member.id, "castigo"))
            await member_fresh.remove_roles(castigo_role, reason="Castigo Anti-Spam expirado (5 min)")
    except Exception:
        pass
...
async def handle_anti_spam(message):
    try:
        ...
        if len(tracker["times"]) >= SPAM_LIMIT:
            ...
            # Aplica castigo de 5 minutos usando o cargo de castigo configurado
            action = "Aviso emitido"
            gd_spam = get_guild_data(message.guild.id)
            castigo_rid = gd_spam.get("castigo_role_id")
            castigo_role = message.guild.get_role(int(castigo_rid)) if castigo_rid else None
            if not castigo_role:
                castigo_role = discord.utils.get(message.guild.roles, name="castigo")
            if castigo_role and member_below_bot(message.guild, member):
                try:
                    _pending_bot_actions.add((message.guild.id, member.id, "castigo"))
                    await member.add_roles(castigo_role, reason=f"Anti-Spam: {SPAM_LIMIT} msgs em {SPAM_WINDOW}s")
                    action = "Castigo aplicado (5 min)"
                    asyncio.create_task(_remove_castigo_after(message.guild, member, castigo_role, 300))
                except Exception:
                    _pending_bot_actions.discard((message.guild.id, member.id, "castigo"))
                    action = "Aviso emitido (sem permissao para castigo)"
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Game(name="agl!help | Anti-Raid ativo"))
    except Exception:
        pass
            ...
            if owner:
                embed_owner = discord.Embed(
                    ...
                    description=(
                        ...
                        f"**1.** Reativar usando a chave de ativacao (`agl!ativar <chave>`)\n"
            ...
            if _URL_PATTERN.search(message.content):
                ...
                if message.author.top_role < bot_top and message.author.id != message.guild.owner_id:
                    ...
                    try:
                        ...
                        pass
                    return
        # ── Sistema PNG: cargo com limite de 1 imagem/minuto ──
        if message.attachments or any(e.image or e.video for e in message.embeds):
            blocked = await _handle_png_system(message)
            if blocked:
                return
...
async def ban(ctx, member: discord.Member = None, *, reason="Sem motivo informado"):
    ...
    if not allowed:
        p = await get_prefix(bot, ctx.message)
        return await reply_and_delete(ctx, error_embed(
            f"Limite de banimentos atingido ({limit} bans). Use {p}resetlimit.", ctx.guild))
    try:
    ...
    # Monta contador de banimentos do moderador
    gd_ban = get_guild_data(ctx.guild.id)
    now_ban = datetime.now(timezone.utc).timestamp()
    ban_counter_txt = ""
    for role in ctx.author.roles:
        ld = gd_ban["ban_limits"].get(str(role.id))
        if ld:
            used = ld.get("used", 0)
            total = ld.get("limit", 0)
            ban_counter_txt = f"\nJa banio **{used}/{total}** pessoas"
            break
    await reply_and_delete(ctx, success_embed("Banimento", f"{member} foi banido.\nMotivo: {reason}{ban_counter_txt}", ctx.guild))
...
async def _executar_cl(message, gd):
    """Logica central do CL: apaga todas as mensagens do autor no canal instantaneamente."""
    ...
    try:
        ...
        pass
    # Purge instantaneo — bem mais rapido que deletar uma a uma
    try:
        await channel.purge(limit=None, check=lambda m: m.author.id == author.id)
    except (discord.Forbidden, discord.HTTPException):
        # Fallback para mensagens com mais de 14 dias (purge nao funciona)
        try:
...
def _build_help_embed(guild, p, category: str) -> discord.Embed:
    ...
    if category == "inicio":
        e = create_embed(guild, "agl! — Inicio")
        e.description = (
    ...
    if category == "moderacao":
        e = create_embed(guild, "agl! — Moderacao")
    ...
    if category == "blacklist":
        e = create_embed(guild, "agl! — Blacklist")
    ...
    if category == "seguranca":
        e = create_embed(guild, "agl! — Seguranca")
    ...
    if category == "painelcargos":
        e = create_embed(guild, "agl! — Painel de Cargos")
    ...
    if category == "permissoes":
        e = create_embed(guild, "agl! — Permissoes")
        ...
        e.add_field(name="Limite de Bans por Cargo", value=(
            f"`{p}addpermban @cargo <N>` — definir limite de bans por cargo\n"
    ...
    if category == "boasvindas":
        e = create_embed(guild, "agl! — Boas-vindas")
        e.add_field(name="Comandos", value=(
    ...
    if category == "sorteio":
        e = create_embed(guild, "agl! — Sorteio")
        e.add_field(name="Comandos", value=(
    ...
    if category == "util":
        e = create_embed(guild, "agl! — Utilitarios")
        e.add_field(name="Voice", value=(
            f"`{p}limitcall [#canal] <0-99>` — alterar limite de usuarios em canal de voz (qualquer um)"
        ), inline=False)
        e.add_field(name="PNG (imagens)", value=(
            f"`{p}setpngrole @cargo` — define o cargo PNG (so pode enviar 1 imagem/min)\n"
            f"`{p}ticketbanner <url>` — define o banner do painel de tickets via link"
        ), inline=False)
        return e

    if category == "configuracao":
        e = create_embed(guild, "agl! — Configuracao")
    ...
    if category == "geral":
        e = create_embed(guild, "agl! — Geral")
        e.add_field(name="Comandos", value=(
...
class HelpSelect(discord.ui.Select):
    def __init__(self, guild, p, requester_id):
        ...
        options = [
            ...
            discord.SelectOption(label="Utilitarios",      value="util",          description="Limitcall, PNG, ticketbanner e mais"),
...
async def setarcargo(ctx, membro: discord.Member = None):
    ...
    if not membro:
        return await reply_and_delete(ctx, error_embed("Mencione um membro. Ex: `agl!setarcargo @membro`", ctx.guild))
    ...
    if not panel_roles_ids:
        return await reply_and_delete(ctx, error_embed("Nenhum cargo configurado no painel. Use `agl!addcargopanel @cargo`.", ctx.guild))

    panel_roles = [ctx.guild.get_role(int(rid)) for rid in panel_roles_ids if ctx.guild.get_role(int(rid))]
    if not panel_roles:
        return await reply_and_delete(ctx, error_embed("Os cargos configurados nao existem mais. Reconfigure com `agl!addcargopanel`.", ctx.guild))
...
# ─── resetlimit (apenas dono do bot) ─────────────────────────────────────────

@bot.command(name="resetlimit")
async def resetlimit(ctx):
    """Reseta os contadores de limite de ban de todos os cargos. Apenas dono do bot."""
    if not is_bot_owner(ctx.author.id):
        return await reply_and_delete(ctx, error_embed("Apenas o dono do bot pode usar este comando.", ctx.guild))
    gd = get_guild_data(ctx.guild.id)
    now = datetime.now(timezone.utc).timestamp()
    count = 0
    for rid in gd["ban_limits"]:
    ...
    embed = create_embed(ctx.guild, "Limites de Ban Resetados")
    embed.description = (
        f"Os contadores de banimento foram resetados para **{count}** cargo(s).\n"
        f"Todos os usuarios voltaram a ter o limite completo disponivel.\n"
        f"**Resetado por:** {ctx.author.mention} (dono do bot)"
    )
    try:
        await ctx.message.delete()
    except Exception:
...
# ─── criarantiban — cria cargo antiban abaixo do bot ─────────────────────────

@bot.command(name="criarantiban")
async def criarantiban(ctx):
    """
    Cria o cargo Anti-Ban posicionado logo abaixo do cargo do bot.
    Esse cargo NAO tem permissao de banir.
    ...
    existing = discord.utils.get(ctx.guild.roles, name="Anti-Ban")
    if existing:
        return await reply_and_delete(ctx, error_embed(
            f"O cargo **Anti-Ban** ja existe: {existing.mention}\n"
            f"Use `{ctx.prefix}setantibanrole @Anti-Ban` para configura-lo como cargo padrao.", ctx.guild))

    try:
        # Cria o cargo sem permissao de ban
        antiban_role = await ctx.guild.create_role(
            name="Anti-Ban",
            ...
            permissions=discord.Permissions(
                ...
                connect=True,
                speak=True,
                # ban_members e administrator ficam False por padrao
            ),
            reason=f"Cargo Anti-Ban criado por {ctx.author}"
        )
    except discord.Forbidden:
        return await reply_and_delete(ctx, error_embed("Sem permissao para criar cargos.", ctx.guild))

    # Posiciona logo abaixo do cargo do bot
    bot_top = ctx.guild.me.top_role
    try:
        new_pos = max(1, bot_top.position - 1)
        await antiban_role.edit(position=new_pos, reason="Posicionando Anti-Ban abaixo do bot")
    except Exception:
        pass

    # Salva como antiban_role_id
    gd = get_guild_data(ctx.guild.id)
    gd["antiban_role_id"] = str(antiban_role.id)
    update_guild_data(ctx.guild.id, gd)

    embed = create_embed(ctx.guild, "Cargo Anti-Ban Criado")
    embed.description = (
        f"**Cargo:** {antiban_role.mention}\n"
        f"**Posicao:** Abaixo do cargo do bot\n"
        f"**Permissao de banir:** Nao\n"
        f"**Obs:** Apenas o dono e cargos selecionados podem banir.\n\n"
        f"O cargo sera dado automaticamente a novos membros ao entrar.\n"
        f"Use `{ctx.prefix}setantibanrole` para alterar o cargo se necessario."
    )
    try:
        await ctx.message.delete()
    except Exception:
...
# ─── limitcall — altera limite de usuarios em canal de voz ───────────────────

@bot.command(name="limitcall")
async def limitcall(ctx, canal: discord.VoiceChannel = None, limite: int = None):
    """
    Altera o limite de usuarios em um canal de voz.
    Qualquer um pode usar este comando.
    Uso: limitcall #canal <numero> (0 = sem limite)
    """
    if not ctx.guild:
        return
    if canal is None:
        # Tenta usar o canal de voz atual do autor
        if ctx.author.voice and ctx.author.voice.channel:
            canal = ctx.author.voice.channel
        else:
            p = await get_prefix(bot, ctx.message)
            return await reply_and_delete(ctx, error_embed(
                f"Mencione um canal de voz ou entre em um canal.\nUso: `{p}limitcall #canal <0-99>`", ctx.guild))
    if limite is None or limite < 0 or limite > 99:
        p = await get_prefix(bot, ctx.message)
        return await reply_and_delete(ctx, error_embed(
            f"Informe um numero entre 0 e 99 (0 = sem limite).\nUso: `{p}limitcall #canal <0-99>`", ctx.guild))
    try:
        await canal.edit(user_limit=limite, reason=f"Limite alterado por {ctx.author}")
    except discord.Forbidden:
        return await reply_and_delete(ctx, error_embed("Sem permissao para alterar este canal de voz.", ctx.guild))
    limite_txt = "sem limite" if limite == 0 else str(limite)
    embed = create_embed(ctx.guild, "Limite de Call Alterado")
    embed.description = (
        f"**Canal:** {canal.mention}\n"
        f"**Novo limite:** {limite_txt} usuario(s)\n"
        f"**Alterado por:** {ctx.author.mention}"
    )
    await reply_and_delete(ctx, embed)

# ─── ticketbanner — define o banner do painel de tickets via comando ──────────

@bot.command(name="ticketbanner")
async def ticketbanner(ctx, url: str = None):
    """
    Define o banner do painel de tickets informando um link de imagem.
    Uso: ticketbanner <url_da_imagem>
    Apenas o dono do servidor pode usar.
    """
    if not is_server_owner(ctx):
        return await reply_and_delete(ctx, error_embed("Apenas o dono do servidor pode usar este comando.", ctx.guild))
    if not url:
        p = await get_prefix(bot, ctx.message)
        return await reply_and_delete(ctx, error_embed(
            f"Informe o link da imagem.\nUso: `{p}ticketbanner https://link-da-imagem.png`\n"
            f"Para remover o banner use: `{p}ticketbanner remover`", ctx.guild))
    gd = get_guild_data(ctx.guild.id)
    if url.lower() in ("remover", "none", "remove", "off"):
        gd["ticket_config"]["banner_url"] = None
        update_guild_data(ctx.guild.id, gd)
        return await reply_and_delete(ctx, success_embed("Banner Removido", "O banner do painel de tickets foi removido.", ctx.guild))
    # Valida que parece uma URL
    if not url.startswith(("http://", "https://")):
        return await reply_and_delete(ctx, error_embed("URL invalida. O link deve comecar com http:// ou https://", ctx.guild))
    gd["ticket_config"]["banner_url"] = url
    update_guild_data(ctx.guild.id, gd)
    embed = create_embed(ctx.guild, "Banner do Ticket Atualizado")
    embed.description = f"O banner do painel de tickets foi definido para:\n{url}"
    try:
...
# ─── Sistema PNG ──────────────────────────────────────────────────────────────
# Cargo PNG: membros com este cargo so podem enviar 1 imagem por minuto.
# Ao enviar, o cargo e removido e devolvido apos 60 segundos.

_png_tracker: dict[tuple, float] = {}  # (guild_id, user_id) -> timestamp do ultimo envio

async def _handle_png_system(message: discord.Message) -> bool:
    """
    Verifica se o autor tem o cargo PNG e se enviou imagem.
    Se sim, remove o cargo temporariamente (1 minuto) e retorna True se bloqueou.
    """
    try:
        if not message.guild or not isinstance(message.author, discord.Member):
            return False
        gd = get_guild_data(message.guild.id)
        png_role_id = gd.get("png_role_id")
        if not png_role_id:
            return False
        png_role = message.guild.get_role(int(png_role_id))
        if not png_role or png_role not in message.author.roles:
            return False

        # Verifica se a mensagem tem imagem/video ou conteudo sensivel
        has_media = bool(message.attachments)
        if not has_media:
            for emb in message.embeds:
                if emb.image or emb.video:
                    has_media = True
                    break
        if not has_media:
            return False
        ...
        now = datetime.now(timezone.utc).timestamp()
        last_sent = _png_tracker.get(key, 0)
        ...
        if now - last_sent < 60:
            ...
            try:
                warn = await message.channel.send(
                    f"{message.author.mention} Voce so pode enviar 1 imagem por minuto. "
        ...
        # Permite o envio — registra o timestamp e remove o cargo por 1 minuto
        _png_tracker[key] = now
        try:
            _pending_bot_actions.add((message.guild.id, message.author.id, "png"))
            await message.author.remove_roles(png_role, reason="PNG: imagem enviada — cooldown 1 min")
        except Exception:
            _pending_bot_actions.discard((message.guild.id, message.author.id, "png"))
            return False

        asyncio.create_task(_restore_png_role(message.guild, message.author, png_role, 60))

        # Log
        embed_log = create_embed(message.guild, "PNG — Imagem Enviada")
        embed_log.description = (
            f"**Usuario:** {message.author.mention} ({message.author.id})\n"
            f"**Canal:** {message.channel.mention}\n"
            f"**Cargo PNG removido por:** 60 segundos\n"
            f"**Horario:** <t:{int(now)}:F>"
        )
        await send_log(message.guild, "anti_gore", embed_log)
        return False  # Permite a mensagem passar
    except Exception as e:
        print(f"[ERRO] _handle_png_system: {e}", flush=True)
        return False

async def _restore_png_role(guild, member, png_role, delay: int):
    """Devolve o cargo PNG apos o cooldown."""
    await asyncio.sleep(delay)
    try:
        member_fresh = guild.get_member(member.id)
        if member_fresh and png_role not in member_fresh.roles:
            _pending_bot_actions.add((guild.id, member.id, "png"))
            await member_fresh.add_roles(png_role, reason="PNG: cooldown expirado — cargo devolvido")
    except Exception:
        pass
